...
같은 설정으로 이어서 돌릴 때는 `--resume_completed`를 붙이면 이미 완료된 compatible row를 건너뜁니다. 이때 commit, gate version, model, epoch, batch size, lambda, seed evidence가 맞지 않으면 안전하게 다시 학습합니다.
`--exp` 이름이 틀리면 데이터 로딩 전에 즉시 에러가 납니다. 유효한 이름은 `Baseline`, `Masking Cons Reg`, `Naive Swap`, `Validity-Gated`, `Strict-Gated`, `Strict-Matched`, 또는 `Strict_lam=<positive_float>` 형식입니다.
